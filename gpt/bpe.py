...
## scripts for training on specific files

TEST_FIXTURES_DIR = Path(__file__).parent.parent / "tests" / "fixtures"
DATA_DIR = Path(__file__).parent.parent / "data"

def train_bpe_tinystories_5M():
    input_path = TEST_FIXTURES_DIR / "tinystories_sample_5M.txt"
    vocab_size = 10000
    special_tokens = ["<|endoftext|>"]
    train_bpe(input_path, vocab_size, special_tokens, 16)
...
def train_bpe_owt():
    ...
    vocab_size = 32000
    special_tokens = ["<|endoftext|>"]
    vocab, merges = train_bpe(input_path, vocab_size, special_tokens, 32)
    with open(DATA_DIR / "owt_train_bpe_vocab.pkl", "wb") as f:
        pickle.dump(vocab, f)
    with open(DATA_DIR / "owt_train_bpe_merges.pkl", "wb") as f:
        pickle.dump(merges, f)
    
def profile_bpe_tinystories():
    """ PROFILE BPE TRAINING ON TINYSTORIES """
    ...
    s = io.StringIO()
    sortby = 'cumulative'  # Or 'tottime', 'calls'
